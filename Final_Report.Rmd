...
I also conducted more detailed analysis at several bacterial phyla too see if there are notable trends at the species, genus, or other higher-resolution levels. At this more detailed lever, I expected to find phylogenetic differences between bacteria found in multiple sclerosis patients versus healthy controls. I figured that if there was a difference between the microbiomes of these two groups, then there should be distinct phylogenetic clades that contain wither multiplesclerosis patient or healthy control bacteria. The main purpose of this study was to discover what trends there are among multiple sclerosis patient and healthy control microbiomes, in both general and detailed terms.
...
DADA2 was used for inter-sequence analysis and cleaning, such as forming OTUs and resolving minor sequence differences. The DADA2 pipeline begins with filtering paired fastq files by trimming them to a specific length and removing sequences that are too short, then further filtering based on number of expected errors, quality score, and number of ambiguous bases [@callahan2016]. Because I was working with pyrosequencing data that was quite lengthy, I set my maximum sequence length to 700 base pairs. Next, DADA2 can remove chimeras, which are sequences that are mare up of two parent sequences that result from sequencing errors. Lastly, it merges paired forward and reverse reads after all the sequences have been cleaned and de-noised, so that DADA2 can be extremely strict and require exact overlap.
